...
public class MarketService : IMarketService
{
    ...
    public async Task<Quote[]> GetAllUSStocksAsync(int? count = null, CancellationToken ct = default)
    {
        ...
        var countParam = count?.ToString() ?? "5000";
        var fields = "logoUrl,longName,shortName,regularMarketChange,regularMarketChangePercent,regularMarketPrice,ask,bid,regularMarketOpen";
        ...
        return quotesArray.EnumerateArray().Select(item => new Quote()
        {
            ...
            bid = ExtractDecimal(item, "bid"),
            open = ExtractDecimal(item, "regularMarketOpen"),
